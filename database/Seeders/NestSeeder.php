...
class NestSeeder extends Seeder
{
    ...
    private function createSourceEngineNest(?array $nest = null)
    {
        if (is_null($nest)) {
            $this->creationService->handle([
                'name' => 'nodejs',
                'description' => 'Includes support for most Source Dedicated server wa bot.',
            ], 'terbo@pterodactyl.io');
        }
    }
    ...
    private function createVoiceServersNest(?array $nest = null)
    {
        if (is_null($nest)) {
            $this->creationService->handle([
                'name' => 'python',
                'description' => 'Voice servers such as python.',
            ], 'terbo@pterodactyl.io');
        }
    }
